...
function AssetFacts({ asset }: { asset: AssetDetail }) {
  ...
  return <>
    ...
    {asset.last_error && <Banner tone="warning" title="마지막 수집 실패"><UntrustedText value={asset.last_error} /> · 이전 조사 결과를 보입니다</Banner>}
    {asset.check_error && <Banner tone="warning" title="배포판 대조 실패"><UntrustedText value={asset.check_error} /></Banner>}

    <dl className="m-0 grid grid-cols-2 gap-x-4 gap-y-3 text-sm sm:grid-cols-3 lg:grid-cols-6">
      <Fact label="OS" value={<UntrustedText value={os} max={120} fallback="—" />} />
      <Fact label="실행 커널" value={<span className="inline-flex flex-wrap items-center gap-1.5">
        <span className="min-w-0 font-mono"><UntrustedText value={asset.kernel_running_version} max={120} fallback="—" /></span>
        {asset.reboot_pending && <Badge tone="orange" title={`설치된 최신 커널 ${revealHidden(asset.kernel_newest_version ?? '—')}`}>재부팅 대기</Badge>}
      </span>} />
      <Fact label="방법 · 호스트" value={<>{METHOD_LABEL[asset.method] ?? asset.method} · <span className="font-mono"><UntrustedText value={asset.host} max={120} fallback="미기록" /></span></>} />
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {asset.key_packages.length === 0 ? <span className="text-xs text-ink-muted">없음</span> : (
        <dl className="m-0 grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-3 lg:grid-cols-4">
          {asset.key_packages.map((p, i) => <Fact key={`${i}-${p.name}`} label={<UntrustedText value={p.name} max={120} />} value={<span className="font-mono text-xs"><UntrustedText value={p.version} max={120} /></span>} />)}
        </dl>
      )}
    </div>
      ...
      {asset.images.length === 0 ? <span className="text-xs text-ink-muted">없음</span> : (
        <ul className="m-0 flex list-none flex-col gap-1 p-0 text-xs" aria-label="컨테이너 이미지">
          {asset.images.map((img, i) => <li key={`${i}-${img.container}-${img.image}`} className="break-all">
            <span className="font-medium"><UntrustedText value={img.container} max={120} /></span> · <span className="font-mono"><UntrustedText value={img.image} /></span>
          </li>)}
        </ul>
      )}
    </div>
